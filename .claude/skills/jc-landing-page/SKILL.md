...
## 10. 산출물 검증 — jc-redteam 'LP 마케팅 패널' (필수, 생략 불가)

**1차 HTML이 완성되면 곧바로 jc-redteam의 'LP 마케팅 패널' 모드로 검증한다.** 검증 없이 1차를 최종으로 제출하지 않는다. (구 Phase 10 = MARKETING_AGENT 시스템을 jc-redteam으로 흡수.)

- **발동 시점**: Phase 0 인풋 체크리스트 완료 → 본 표준대로 1차 HTML 생성·노출 직후 자동. 작업자가 "검증 생략"을 명시하지 않은 경우.
- **진입 고지(한 줄)**: "1차 산출물 완성. 5인 마케터 + 3인 고객 패널로 최종본을 도출합니다."
- **절차·페르소나(5인 마케터+3인 고객)·6단계 워크플로우·안티패턴·압축 모드·검수 체크리스트**: 전부 정본 **`jc-redteam/references/lp-marketing-panel.md`** 참조.
- **산출**: 1차 HTML 보존 + 액션 반영 **v2 HTML(별도 파일, `[campaign]_lp_v2.html`)** + '변경 근거' 섹션(어느 페르소나의 어떤 지적을 반영했는지).

> 일반 결론·문서 검증은 jc-redteam 기본 3축, **LP 산출물**은 위 'LP 마케팅 패널' 모드를 쓴다.
