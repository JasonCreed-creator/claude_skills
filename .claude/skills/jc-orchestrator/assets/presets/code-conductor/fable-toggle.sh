#!/usr/bin/env bash
# fable-toggle.sh → ~/.local/bin/fable 로 설치 — 오케스트레이션 스위치 (런처가 아니라 토글)
#
...
#   3. ~/.bashrc 의 source ~/.claude/fable/env.sh 한 줄    (리매핑 — 선택)
...
case "${1:-status}" in
  on)
    ...
    echo "오케스트레이션 ON — 다음 claude 세션부터 적용됩니다."
    ;;
  off)
    echo off > "$STATE"
    ln -sfn "$FABLE_DIR/empty.md" "$FABLE_DIR/active.md"
    echo "오케스트레이션 OFF — 다음 claude 세션부터 일반 모드입니다."
    ;;
  status)
