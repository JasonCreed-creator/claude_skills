#!/usr/bin/env python3
# code-conductor 강제 게이트 (PreToolUse) — jc-orchestrator 프리셋
#
# 메인 에이전트가 한 턴(prompt_id)에 코드 파일을 LIMIT개까지만 직접 수정하게 허용하고,
# 초과분부터 차단하며 위임 지시를 돌려준다. Bash를 통한 코드 파일 수정은 항상 차단.
# 서브에이전트(payload에 agent_id/agent_type 존재)는 전부 통과 — 위임이 실행 경로다.
#
# fable off 상태(~/.claude/.fable-state != on)면 즉시 통과. 등록은 settings.json에
# 1회(로딩 코드), 동작 여부는 스위치가 지배한다.
# 오류 시 fail-open(통과) — 게이트 버그가 세션을 마비시키지 않게 한다.
#
# 설치·실측 검증 절차: INSTALL.md §4 (prompt_id 필드는 구버전 Claude Code에 없음 — §0 확인)
...
LIMIT = 2  # 턴당 직접 수정 허용 코드 파일 수 (조정 가능 파라미터)
...
def main():
    ...
    if tool == "Bash":
        ...
        if writes_redirect or (writes_inplace and touches_code):
            deny(
                "[게이트] 메인 에이전트는 Bash로 코드 파일을 수정할 수 없습니다. "
                "코드 수정은 Edit/Write 도구(턴당 %d개 파일까지)를 쓰거나 서브에이전트에 위임하세요: "
                "일반 구현·수정 → 실행 에이전트(상위 계열), 심층 추론·설계 → deep-reasoner, 단순 잡무 → runner." % LIMIT
            )
        allow()
    ...
    deny(
        "[게이트] 이번 턴에 메인 에이전트가 직접 수정한 코드 파일이 이미 %d개입니다 (%s). "
        "추가 코드 수정은 반드시 서브에이전트에 위임하세요: 일반 구현·수정·테스트·리뷰 → 실행 에이전트(상위 계열), "
