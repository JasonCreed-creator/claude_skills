...
> 생성: 2026-07-25 · 상태: 확정본 (2026-08-18 리멤버 전환 P2 타깃 동기)
> 기준: jc-workspace-ops v1.1.0 세션 층 흡수 (2026-07-25) + 리멤버 전환 D1/D2 (2026-08-18)
> 소스: `library` (26종)
> 생성기: `scripts/build_routing_map.py` — 손으로 수정하지 말 것. description 변경 시 재실행.
> 2026-08-18 타깃 동기(리멤버 전환): mice-estimate description(v3.0.0) 미러 + 버전 헤딩 갱신(jc-design-system v1.4.0·jc-theme-factory v1.1.1·mice-dashboard v2.0.4·jc-brand-styling v1.0.2). 잔여 드리프트 정리분 반영 — 활성 사양부의 아카이브 오버레이(`mc`·`darktrace`) 참조 제거. 줄바꿈 LF 정규화(구 CRLF 혼재 116행). 전체 재생성(run-of-show·aftermath·cinematic §2 행 편입, 26→28종 정합)은 별도 사이클로 이월(두 미러 정합 백로그).
