...
> **2026-07-12 증분**: CP3 인테이크 반영 — registry **재생성(22→25종)**: `jc-orchestrator` 스냅숏 최초 편입 + 신규 2종 `jc-brand-discovery`·`jc-workspace-ops`. §2 행 2건 추가(brand-discovery=브랜드 정체성 발굴 / workspace-ops=Drive 운영·정리). §3 판별 추가: §3-3에 '브랜드 *정체성 발굴* 인터뷰→jc-brand-discovery' 분기, §3-8 "정리·파일" 신설(파일·문서 *정리* vs 내용 구조화 분기). 기존 5종 버전 범프(jc-doc-coauthor v1.0.1·jc-redteam v1.2.2·jc-skill-forge v1.0.3·mice-market-intel v1.0.4·jc-orchestrator v1.1.2)는 description *내용* 무변경 → §2·§3 기존 행 불변. 신규 2종 description 모두 브리프 게이트 역참조 문구('실행형 지시는 실행 전 jc-prompt-builder 브리프를 거친다.') 보유.

> **2026-07-25 증분**: jc-workspace-ops **v1.1.0**(세션 층 흡수) 반영 — registry 재생성(25→**26종**, label "jc-workspace-ops v1.1.0 세션 층 흡수"). §2 workspace-ops 행 트리거에 세션 어휘 6종('체크인'·'체크아웃'·'PROGRESS'·'세션 이어서'·'이어서 작업'·'세션 연속') 추가, §3-8에 세션 리추얼 판별 추가. 재생성에 **jc-cinematic-html v1.0.0 최초 편입**(같은 날 별도 세션 배치분, PROGRESS 병합대기) — 그 §2 행·판별은 해당 배치 체크아웃에서 처리하며 본 증분은 편입 사실만 기록.
...
| 브랜드 정체성 발굴(다중세션 인터뷰) | jc-brand-discovery | '브랜드 발굴'·'브랜드 정체성'·'브랜드 인터뷰'·'행사명/네이밍'·'슬로건/태그라인'·'아키타입/보이스' — 인터뷰로 *정체성을 발굴*하는 상류 전략(네이밍·슬로건·비주얼 방향 이전 단계) | 브랜드 정체성 브리프(.md) + state.json 체크포인트 |
...
| Drive 폴더·파일 운영·정리 | jc-workspace-ops | '드라이브/폴더/파일 정리'·'폴더 구조/위계'·'공유 권한 점검'·'중복/최신본'·'트래커/체크리스트'·**'체크인'/'체크아웃'·'PROGRESS'·'세션 이어서/이어서 작업/세션 연속'**(세션 연속성 — 2파일 리추얼, v1.1.0 §5) — 산출물의 *저장·배치·점검·청소* 운영 층(생성이 아님) | 표준 폴더 트리(+세션 2파일 템플릿 전개)·배치·권한 점검 결과 + 시트 트래커 (권한변경·삭제·이동 등 고위험 조작은 안내만) |
...
- 판별 축은 파일 유무가 아니라 **작업 성격**이다 (factory는 파일 첨부 케이스도 명시 트리거로 보유):
  - 브랜드 **정체성 발굴** 인터뷰(발굴·네이밍·슬로건 상류 전략 — 아직 색·토큰·테마 이전 단계) → jc-brand-discovery
...
### 3-8. "정리 · 파일 · 문서 정리"
- 파일·폴더·드라이브 **정리·배치·저장 위치**(공유 권한 점검·트래커 포함) → jc-workspace-ops
- '정리해줘'는 **대상**으로 가른다: 파일·폴더 배치·저장이면 jc-workspace-ops / 회의·transcript 내용 구조화면 mice-meeting-minutes / 메일·공지 등 커뮤니케이션 글이면 jc-comms / 산문 문서 집필·구조화면 jc-doc-coauthor. '문서 정리'는 *파일링(위치)인가 내용(집필)인가*가 판별축이다.
- '체크인'·'체크아웃'·'세션 이어서/이어서 작업'(프로젝트 폴더의 세션 시작·종료·재개 리추얼) → jc-workspace-ops §5 (정본: `00_CONTEXT/session-protocol.md`). 단독 'PROGRESS' 언급은 과발동 주의 — 프로젝트 폴더의 `PROGRESS.md` 파일 문맥일 때만 workspace-ops이고, 진행 *보고·업데이트 글* 작성이면 jc-comms.
