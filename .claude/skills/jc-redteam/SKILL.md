...
7. **증거주의 (Evidence-Before-Claim)** — 감사관의 지적에도 근거가 필요하다. 재검증 가능한 결함(숫자·계산·참조 경로·제품/버전 팩트·인용)은 **재계산·재실행·원문 대조로 직접 확인한 뒤** 보고하고, 확인 증거(원문 인용·계산 과정·실행 결과)를 결함 항목에 병기한다. 직접 확인이 불가능한 항목은 "미검증 추정"으로 구분 표기한다 — 검증되지 않은 지적을 검증된 것처럼 쓰는 순간 레드팀 자체가 신뢰를 잃는다.
...
### 옵션 강도 티어 (opt-in 상향 — 기본 2모드 동작 불변, 명시 요청·고부담 시에만 발동)
- **이중 독립 감수** (납품 직전·고부담 산출물에 opt-in): Deep Audit를 서로의 결과를 보지 않는 독립 감수자 2인이 동일 3축 루브릭으로 각자 수행 → **둘 다 통과해야 출하**(한쪽만 잡은 결함도 실재로 간주). 불일치 시 결함 수정 후 신선한 감수 2회를 수렴까지 재실행(상한 3회, 초과 시 사용자 에스컬레이션).
- **결정 카운슬** (의사결정 검증 요청 시 opt-in): 다경로·트레이드오프 판단에 4관점 렌즈(Architect=장기 정합성 / Skeptic=전제·가정 격파 / Pragmatist=실행 현실 / Critic=엣지·실패 모드)를 세워 다각 공격. 기존 3축(§5)을 대체하지 않고 각 렌즈에 3축을 배분하는 결합 방식 → 관점별 입장·최강 반론을 노출한 뒤 종합 판정(§6)으로 수렴.
...
| 축 | 핵심 질문 | 세부 점검 |
|----|----------|----------|
| **1. 내용 검증** | 이 주장은 참인가, 논리는 성립하는가? | 사실 오류 / 논리 비약·인과 오류 / 근거 부족 / 숨은 가정 / 인지 편향(확증·매몰비용·앵커링·생존자·낙관) / 누락된 관점 / 내부 모순 / **제품·버전 팩트 실검증**(폐기 모델 ID·구버전·검증일 누락 — RULE-VERSION-FACTS) |
...
| v1.1.0 | 2026-06 (정확 일자 미기록) | LP 마케팅 패널 모드 추가 — `references/lp-marketing-panel.md` (jc-landing-page 산출물 전용 5인 마케터+3인 고객 도메인 검증). 이력 행이 당시 누락됐다가 2026-07-03 아카이브 스냅샷 실측으로 확정 소급 기재. |
