...
`scripts/build_showcase.py`로 **시그니처 + 등록된 모든 클라이언트 오버레이**를 라이트/다크 카드로 렌더한 단일 HTML을 생성해 사용자에게 보여준다. 임의 테마 10종을 나열하는 게 아니라, 실제 등록된 오버레이(personal·remember·confex…)를 보여준다. 아카이브(deprecated) 오버레이는 쇼케이스에서 제외한다.
...
- v1.1.1 (2026-08-18): 리멤버 전환 잔여 드리프트 정리 — §1 쇼케이스 본문의 등록 오버레이 예시에서 아카이브분(`mc`·`darktrace`) 제거, 쇼케이스 제외 원칙 명문화. 값·로직 무변경.
