...
**자산 정의 원칙**: 본 스킬은 어떤 회사·개인의 식별 정보도 하드코딩하지 않는다. 양식 명칭('M&C', '리멤버')만 식별자로 보존되며, 공급자·고객사·담당자·연락처 등 모든 식별 정보는 **외부 주입 변수**로 처리된다. 산출물 본문에는 사용자가 매번 주입하는 변수만 들어간다.
...
8. **양식 식별자 외 회사명 금지**: 본문·산출물에 구체 회사 상호를 작성하지 않는다. 양식 식별자(`M&C 견적서`·`리멤버 견적서`)만 양식 명칭으로 사용. 공급자·고객사 식별 정보는 모두 외부 주입 변수 (Step 3.5 슬롯).
