...
6. **어투·표기·금칙**: 기획자님 코퍼스(수주 제안서 10건) 증류 문법은 `references/proposal-voice.md`가 정본이다 — 헤드라인 한국어 어투 규칙·번역투 금칙(§3), 숫자·실적 표기(§4), 리스크 화법(§5), 금칙 리스트(§6, RFP 계량화 지침 시 조건부 금칙 포함). 홀드아웃 검증 통과 전까지 본 §3와 충돌 시 본 §3 우선(1차 홀드아웃 P05 2026-09-25 부분 통과 — 우선권 유지, 조건은 proposal-voice §9).
...
10. **어투**: `references/proposal-voice.md` §3-4 점검 5항목(주어 시작 0 · 피동 직역 0 · "~을 통해 ~를 제공" 슬라이드당 1회 이하 · 콜론/세미콜론/대시 0(상대 일정 D-/W-·수치 범위의 하이픈은 예외, 내비 헤더 제외) · 30자 초과 헤드라인 0) 통과.
