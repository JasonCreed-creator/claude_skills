...
6. **어투·표기·금칙**: 기획자님 코퍼스(수주 제안서 10건) 증류 문법은 `references/proposal-voice.md`가 정본이다 — 헤드라인 한국어 어투 규칙·번역투 금칙(§3), 숫자·실적 표기(§4), 리스크 화법(§5), 금칙 리스트(§6). 홀드아웃 검증 통과 전까지 본 §3와 충돌 시 본 §3 우선.
...
**표준 7섹션 (50p 이내, 배점 역설계로 비중 조정)**: ①표지·목차 ②행사 개요·컨셉(발주처·사업 분석 → 추진 전략·강점 포함) ③회사 소개·레퍼런스 ④운영 계획 ⑤크리에이티브 ⑥KPI/성과관리 ⑦예산/부록 — 회사소개는 모든 제안서에서 앞단(`references/proposal-voice.md` §2, 2026-09-25 결정). 발주처 목차가 있으면 그것이 우선. 슬라이드별 패턴 매핑: `references/slide-structure.md`.
...
10. **어투**: `references/proposal-voice.md` §3-4 점검 5항목(주어 시작 0 · 피동 직역 0 · "~을 통해 ~를 제공" 슬라이드당 1회 이하 · 콜론/세미콜론/대시 0 · 30자 초과 헤드라인 0) 통과.
