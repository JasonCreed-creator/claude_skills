...
6. **어투·표기·금칙**: 기획자님 코퍼스(수주 제안서 10건) 증류 문법은 `references/proposal-voice.md`가 정본이다 — 헤드라인 한국어 어투 규칙·번역투 금칙(§3), 숫자·실적 표기(§4), 리스크 화법(§5), 금칙 리스트(§6, RFP 계량화 지침 시 조건부 금칙 포함). **홀드아웃 검증 통과(P05 블라인드 재채점 2026-09-25) — 본 §3·§5·§6과 충돌 시 `proposal-voice.md` 우선.** 배점 역설계(§3-1)와 발주처 목차 우선(§5)은 상위 규칙.
...
**표준 7섹션 (50p 이내, 배점 역설계로 비중 조정)**: ①표지·목차 ②행사 개요·컨셉(발주처·사업 분석 → 추진 전략·강점 포함) ③회사 소개·레퍼런스 ④운영 계획 ⑤크리에이티브 ⑥KPI/성과관리 ⑦예산/부록 — 회사소개는 모든 제안서에서 앞단(`references/proposal-voice.md` §2, 2026-09-25 결정). 발주처 목차가 있으면 **순서까지** 그것이 우선(proposal-voice A-1 — P05 RFP는 제안사 현황 → 제안 개요 순). 슬라이드별 패턴 매핑: `references/slide-structure.md`.
...
- 문체: 한국어 개조식 · 명사형 종결 주장 카피 · 주어 생략 · 번역투 금칙(`references/proposal-voice.md` §3-2), 수치·방법론 중심, 회사·개인 식별정보 하드코딩 금지(RULE-NO-COMPANY — 제안사 정보는 기획자가 주입).
...
2. **제출 조건**: 페이지 제한·목차 양식·필수 서식 등 RFP 제출 조건을 재대조했는가. 배점표가 증빙을 요구하는 항목(예: 안전보건관리책임자 지정 배점·지정서 서식)은 해당 슬라이드에 서식명·증빙 위치를 명시했는가.
...
7. **금칙 수식어**: `references/proposal-voice.md` §6 리스트 중 근거(수치·사례·프로세스명) 미동반 0건 — grep으로 후보를 뽑고 사람이 판정.
8. **정합·QC**: 목차-본문 라벨 일치, 번호 연속, 내부 마커("수정필요"·작업 코멘트) 0건.
9. **클로징**: 마지막 장이 사업 고유 메시지로 끝난다 — 회사 태그라인·연락처만으로 끝내지 않는다.
10. **어투**: `references/proposal-voice.md` §3-4 점검 6항목(주어 시작 0 · 피동 직역 0 · "통해" 슬라이드당 2회 이상 0 · 콜론/세미콜론/대시 0(상대 일정 D-/W-·수치 범위의 하이픈은 예외, 내비 헤더 제외) · 30자 초과 헤드라인 0 · 한 주장(쉼표·가운뎃점 3항목 나열 0)) 통과.
...
| `references/proposal-voice.md` | 기획자님 코퍼스 증류 문법 — 한국어 어투·번역투 금칙, 숫자·실적 표기, 리스크 화법, 금칙 리스트, 게이트 7~10, 홀드아웃 체크리스트 |
...
- **v3.1.0 (2026-09-25)**: `references/proposal-voice.md` 신설 — 수주 제안서 10건(공공 4·리멤버 MICE 2·리서치 4) 증류, 레드팀 검증. §3-6 어투·표기·금칙 포인터, §5 회사소개 앞배치(③), §6 문체 한국어 어투·번역투 금칙, §8 완료 게이트 7~10 추가. description 무변경
- **v3.1.1 (2026-09-25)**: 1차 홀드아웃(P05, 부분 통과 · 우선권 유지) 반영 — §3-6 결과 병기, §5 발주처 목차 순서 우선 명시, §8 게이트 2 증빙 요구 항목·게이트 10 하이픈 예외. `proposal-voice.md` v1.0.1 동반. description 무변경
- **v3.1.2 (2026-09-25)**: 블라인드 재채점 통과 — §3-6 `proposal-voice.md` **우선권 승격**(v1.1.0), §8 게이트 10 점검 6항목. description 무변경
