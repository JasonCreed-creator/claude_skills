...
> **✅ R1 정합 완료.** 본 표의 HEX는 모두 **라이트 정본 미러**이며 값의 권위는 `signature-tokens.md §1·§1.4·§6`(라이트)다. 다크 모드 매핑 값은 `mode-mapping.md §3·§3.2·§7`이 정본.
> ⚠️ 역할 주의: 아래 `#0A2540`은 **라이트 `--jc-primary`(표지 배경)** 및 **`--jc-data-5`(5순위 차트)** 전용 정본이다. 다크 *페이지 배경*은 `#0A2540`이 아니라 `#0A1220`(mode-mapping.md §3) — 혼동 금지.

### 2.1 영역별 토큰 매핑

| 영역 | 토큰 | HEX (시그니처) | 비고 |
|------|------|---------------|------|
| 표지 배경 | `--jc-primary` | `#0A2540` | Deep Navy (라이트 표지 풀블리드 정본 — 유지) |
...
```
시리즈 5종 사용 (라이트 정본 — signature-tokens.md §1.4 / §6):
1순위 산업: --jc-data-1 (#2962FF)
2순위 산업: --jc-data-2 (#E91E63)
3순위 산업: --jc-data-3 (#FF5722)
4순위 산업: --jc-data-4 (#00E676)
5순위 산업: --jc-data-5 (#0A2540)   ← data-5 Deep Navy 정본(표지 아님). 다크 차트는 §3.2 data-5=#C9CFD8
