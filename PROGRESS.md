...
## [코퍼스 증류] mice-proposal v3.1.1 → v3.1.2 · proposal-voice v1.0.1 → v1.1.0 — 2026-09-25 (블라인드 재채점 통과 → 우선권 승격, Drive→git 단방향)

- 승격 조건 (a) 실행: 생성 A(RFP + 규칙만, 제출본 미열람 전사 검증) · 채점 B(1차 리포트 미열람) · 기준 사전 등록. 라운드 1 미통과(헤드라인 1건 쉼표 3항목) → SKILL §8 게이트 수정 1회 → 라운드 2 통과. 경계: 규범 우위 3 = 최소치, H-4·H-5 변수 조건부. 리포트 `JC_main_claude/corpus/validation/2026-09-25_P05-holdout-blind.md`.
